...
export const liftSelector = (id: number) =>
    createSelector(
...
export const liftsSelector = createSelector(
    [(state: IRootState) => state.lifts.entities],
    (lifts): IFormatedLift[] => lifts.map((lift) => formatLift(lift))
)
...
export const liftExerciseSelector = (exerciseId: number) =>
    createSelector(
        ...
        (exercises, lifts): IFormatedLift | null => {
            ...
            const lift =
                exercise && lifts.find((lift) => lift.id === exercise.liftId)
            return lift && Object.keys(lift).length > 0
                ? formatLift(lift)
                : null
        }
    )
